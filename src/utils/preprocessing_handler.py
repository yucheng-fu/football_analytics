...
class PreprocessingHandler:
    ...
    def preprocess_categorical_columns(self) -> pl.DataFrame:
        ...
        self.df = self.df.with_columns(pl.col("under_pressure").fill_null(False).cast(pl.Int64))
    ...
    def preprocess_outcome_column(self) -> pl.DataFrame:
        ...
        self.df = self.df.with_columns(pl.when(pl.col("outcome").is_null()).then(1).otherwise(0).alias("outcome"))
