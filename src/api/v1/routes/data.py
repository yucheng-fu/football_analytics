...
def inference_frame(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Inference frame generation failed: {exc}") from exc
...
def inference_frame_batch(
    ...
    try:
        frames = [inference_frame_service.build_from_payload(payload) for payload in payloads]
