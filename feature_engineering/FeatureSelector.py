...
class ForwardFeatureSelector:
    ...
    def fit(self, data, label):
        ...
        if self.features_for_selection is None:
            ...
            raise ValueError(
                "features_for_selection should be a subset of data.columns"
            )
        ...
        feature_importance_map = {
            col: imp for col, imp in zip(data.columns, self.feature_importance)
        }
        self.features_for_selection.sort(
            key=lambda x: feature_importance_map[x], reverse=True
        )
        self.selected_features = list(
            set(data.columns) - set(self.features_for_selection)
        )
        ...
        if len(self.selected_features) == 0:
            ...
            previous_score = np.mean(
                cross_val_score(
                    self.estimator,
                    self.data[self.selected_features].values.reshape(-1, 1),
                    self.label.values.ravel(),
                    scoring=self.metric,
                    cv=5,
                )
            )
        ...
        while self.features_for_selection:
            ...
            self.history["features_improvement"].append(
            ...
            if improvement < self.threshold:
                self.my_print(
                    ...
                    % (
                        total_count,
                        total,
                        int(total_count / total * 100),
                        self.selected_features[-count:],
                        improvement,
                    )
                )
        ...
        self.my_print(
            "The number of selected features is [%d/%d] = %d%%"
            % (
                self.history["n_selected"][-1],
                total,
    ...
    def visualize(self, test_scores=None, all_score=None):
        ...
        plt.plot(
            self.history["n_examined"],
            self.history["scores"],
            label="cv scores on selected features: %.4lf" % self.history["scores"][-1],
        )
        if test_scores is not None:
            plt.plot(
                self.history["n_examined"],
                test_scores,
                label="test scores on selected features: %.4lf" % test_scores[-1],
            )
        if all_score is not None:
            plt.plot(
                self.history["n_examined"],
                [all_score] * len(self.history["scores"]),
                linestyle="--",
                label="test scores on all features: %.4lf" % all_score,
            )
    ...
    def get_estimator(self):
        if self.estimator is None:
            ...
            if self.task == "classification":
                self.estimator = lgb.LGBMClassifier(**params, verbosity=-1)
            else:
                self.estimator = lgb.LGBMRegressor(**params, verbosity=-1)
...
class TwoStageFeatureSelector:
    ...
    def fit(self, data, label):
        ...
        self.train_index, self.val_index = self.get_index(
    ...
    def stage2_select(self):
        ...
        if self.task == "classification":
            gbm = lgb.LGBMClassifier(**params, verbosity=-1)
        else:
